...
static int new_map()
{
  ...
  while (world.cur_map->map[pc.x][pc.y] != ter_path) {
    pc.x = rand() % 79;
    pc.y = rand() % 20;

    if (pc.x != 0 && pc.y != 0) {
      while (pc.x != 79 && world.cur_map->map[pc.x][pc.y] != ter_path) {
        pc.x++;
      }
    }
  }

  world.cur_map->map[pc.x][pc.y] = ter_pc;
  printf("PC AT: (%d, %d)\n", pc.x, pc.y);
...
static void print_cost(int m[MAP_Y][MAP_X])
{
  int x, y;
...
static void dijkstra_hiker(pair_t from, pair_t to)
{
  static path_t path[MAP_Y][MAP_X], *p;
  static uint32_t initialized = 0;
  heap_t h;
  ...
  //if the cost of the destination makes it unreachable, return immediately
  if (hikerCostMap[to[dim_y]][to[dim_x]] == INT_MAX/2) {
    hikerMap[to[dim_y]][to[dim_x]] = INT_MAX/2;
    return;
  }
  ...
  while ((p = heap_remove_min(&h))) {
    ...
    if ((p->pos[dim_y] == to[dim_y]) && p->pos[dim_x] == to[dim_x]) {
      /*
      *Drawing path not necessary for this implimentation, but thank you.
      for (x = to[dim_x], y = to[dim_y];
           (x != from[dim_x]) || (y != from[dim_y]);
           p = &path[y][x], x = p->from[dim_x], y = p->from[dim_y]) {
        //place path
      ...
      */
      hikerMap[p->pos[dim_y]][p->pos[dim_x]] = (p->cost) % 100;
      heap_delete(&h);
      return;
    }

    if ((path[p->pos[dim_y] - 1][p->pos[dim_x]    ].hn) && (path[p->pos[dim_y] - 1][p->pos[dim_x]    ].cost > ((p->cost + hikerCostMap[p->pos[dim_y]][p->pos[dim_y]])))) {
      path[p->pos[dim_y] - 1][p->pos[dim_x]    ].cost = ((p->cost + hikerCostMap[p->pos[dim_y]][p->pos[dim_y]]));
      path[p->pos[dim_y] - 1][p->pos[dim_x]    ].from[dim_y] = p->pos[dim_y];
      path[p->pos[dim_y] - 1][p->pos[dim_x]    ].from[dim_x] = p->pos[dim_x];
      heap_decrease_key_no_replace(&h, path[p->pos[dim_y] - 1][p->pos[dim_x]    ].hn);
    }
    if ((path[p->pos[dim_y]    ][p->pos[dim_x] - 1].hn) && (path[p->pos[dim_y]    ][p->pos[dim_x] - 1].cost > ((p->cost + hikerCostMap[p->pos[dim_y]][p->pos[dim_y]])))) {
      path[p->pos[dim_y]][p->pos[dim_x] - 1].cost = ((p->cost + hikerCostMap[p->pos[dim_y]][p->pos[dim_y]]));
      path[p->pos[dim_y]    ][p->pos[dim_x] - 1].from[dim_y] = p->pos[dim_y];
      path[p->pos[dim_y]    ][p->pos[dim_x] - 1].from[dim_x] = p->pos[dim_x];
...
static void dijkstra_rival(pair_t from, pair_t to)
{
  static path_t path[MAP_Y][MAP_X], *p;
  static uint32_t initialized = 0;
  heap_t h;
  ...
  //if the cost of the destination makes it unreachable, return immediately
  if (rivalCostMap[to[dim_y]][to[dim_x]] == INT_MAX/2) {
    rivalMap[to[dim_y]][to[dim_x]] = INT_MAX/2;
    return;
  }
  ...
  while ((p = heap_remove_min(&h))) {
    ...
    if ((p->pos[dim_y] == to[dim_y]) && p->pos[dim_x] == to[dim_x]) {
      ...
      */
      rivalMap[p->pos[dim_y]][p->pos[dim_x]] = (p->cost) % 100;
      heap_delete(&h);
      return;
    }

    if ((path[p->pos[dim_y] - 1][p->pos[dim_x]    ].hn) && (path[p->pos[dim_y] - 1][p->pos[dim_x]    ].cost > ((p->cost + rivalCostMap[p->pos[dim_y]][p->pos[dim_y]])))) {
      path[p->pos[dim_y] - 1][p->pos[dim_x]    ].cost = ((p->cost + rivalCostMap[p->pos[dim_y]][p->pos[dim_y]]));
      path[p->pos[dim_y] - 1][p->pos[dim_x]    ].from[dim_y] = p->pos[dim_y];
      path[p->pos[dim_y] - 1][p->pos[dim_x]    ].from[dim_x] = p->pos[dim_x];
      heap_decrease_key_no_replace(&h, path[p->pos[dim_y] - 1][p->pos[dim_x]    ].hn);
    }
    if ((path[p->pos[dim_y]    ][p->pos[dim_x] - 1].hn) && (path[p->pos[dim_y]    ][p->pos[dim_x] - 1].cost > ((p->cost + rivalCostMap[p->pos[dim_y]][p->pos[dim_y]])))) {
      path[p->pos[dim_y]][p->pos[dim_x] - 1].cost = ((p->cost + rivalCostMap[p->pos[dim_y]][p->pos[dim_y]]));
      path[p->pos[dim_y]    ][p->pos[dim_x] - 1].from[dim_y] = p->pos[dim_y];
      path[p->pos[dim_y]    ][p->pos[dim_x] - 1].from[dim_x] = p->pos[dim_x];
      heap_decrease_key_no_replace(&h, path[p->pos[dim_y]    ][p->pos[dim_x] - 1].hn);
    }
