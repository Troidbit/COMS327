...
{
  return effective_stat[stat_maxhp];
}
...
std::ostream &Pokemon::print(std::ostream &o) const
{
  ...
  o << get_species() << " level:" << level << " "
    ...
    << std::endl;
  o << "         HP:" << effective_stat[stat_maxhp] << std::endl
  ...
  o << "     HPBASE:" << s->base_stat[stat_maxhp] << std::endl
