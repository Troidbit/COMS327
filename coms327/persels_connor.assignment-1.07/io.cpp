...
static void io_wild_display(pokemon_db *p)
{
    WINDOW* win;
    int height = 14;
    int width = 73;
    //int ranpokemove = rand() % 528239;
    int shiny = rand() % 8192 == 0;
    int gender = rand() % 2 == 0;
    int i;

    win = newwin(height, width, 5, 4);
    box(win, height / 2, width / 2);
    mvwprintw(win, 1, 2, "A wild %s appeared!",p->identifier);
    
    if (gender == 1) {
        mvwprintw(win, 3, 2, "Gender: Male");
    }
    else {
        mvwprintw(win, 3, 2, "Gender: Female");
    }
    
    pokemon_stats_db poke_stat;
    stats_db stat;
    
    for (i = 1; i < 6553; i++) {
        if (pokemon_stats[i].pokemon_id == p->id) {
            poke_stat = pokemon_stats[i];
            break;
        }
    }
    for (i = 1; i < 9; i++) {
        if (stats[i].id == poke_stat.id) {
            stat = stats[i];
            break;
        }
    }
    
    int level = abs(world.cur_idx[dim_x] - 199) + abs(world.cur_idx[dim_y] - 199);
    if (level <= 200) {
        level /= 2;
        level = rand() % level + 1;
    }
    ...
    mvwprintw(win, 4, 2, "Level: %d", level);
    if (shiny == 1) {
        mvwprintw(win, 5, 2, "Shiny: Yes");
    }
    else {
        mvwprintw(win, 5, 2, "Shiny: No");
    }
    ...
    int hp = ((((poke_stat.base_stat + hpiv) * 2) * level) / 100) + level + 10;
    int attack = ((((poke_stat.base_stat + atkiv) * 2) * level) / 100) + 10;
    int defense = ((((poke_stat.base_stat + defiv) * 2) * level) / 100) + 10;
    int spa = ((((poke_stat.base_stat + spaiv) * 2) * level) / 100) + 10;
    int spd = ((((poke_stat.base_stat + spdiv) * 2) * level) / 100) + 10;
    int speed = ((((poke_stat.base_stat + speediv) * 2) * level) / 100) + 10;

    mvwprintw(win, 9, 2, "base stat: %d", poke_stat.base_stat);
    mvwprintw(win, 6, 2, "Pokemon Stats:    Health  Attack  Defence SP. A   SP. D   Speed");
    mvwprintw(win, 7, 2, "            %8d%8d%8d%8d%8d%8d",hp,attack,defense,spa,spd,speed);
    mvwprintw(win, 8, 2, "IV Stats:   %8d%8d%8d%8d%8d%8d",hpiv,atkiv,defiv,spaiv,spdiv,speediv);
    
    // version == 19
    // pokemon_moves.pokemon_id == species.id
    // method_id = 1

    std::vector<pokemon_move_db> poke_moves;
    for (i = 1; i <= 528239; i++) {
        if (pokemon_moves[i].version_group_id==18 && pokemon_moves[i].pokemon_id == p->id && pokemon_moves[i].pokemon_move_method_id == 1) {
            poke_moves.push_back(pokemon_moves[i]);
        }
    }

    if (poke_moves.size() < 1) {
        mvwprintw(win, 9, 2, "Moves: No moves");
    }
    else {
        int ranmove1 = rand() % poke_moves.size();
        move_db move1 = moves[(poke_moves.at(ranmove1).move_id)];
        if (poke_moves.size() < 2 || poke_moves.size() == 1) {
            mvwprintw(win, 9, 2, "Moves: %s", move1.identifier);
        }
        else {
            int ranmove2;
            do {
                ranmove2 = rand() % poke_moves.size();
            } while (ranmove2 == ranmove1);
            move_db move2 = moves[(poke_moves.at(ranmove2).move_id)];

            mvwprintw(win, 9, 2, "Moves:\n  [%s]\n  [%s]", move1.identifier, move2.identifier);
        }
    }
...
static void io_random_wild() {
    int ranpoke = rand() % 1093;

    pokemon_db p = pokemon[ranpoke];
...
uint32_t move_pc_dir(uint32_t input, pair_t dest)
{
  ...
  if (world.cur_map->map[world.pc.pos[dim_y]][world.pc.pos[dim_x]] == ter_grass) {
      int randomEncounterChance = rand() % 10 == 0;
