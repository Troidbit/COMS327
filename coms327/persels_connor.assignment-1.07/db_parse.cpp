...
void db_parse(bool print)
{
    ...
    for (i = 1; i <= 6553; i++) {
        fgets(line, 80, f);
        pokemon_stats[i].pokemon_id = atoi(next_token(line, ','));
        pokemon_stats[i].id = atoi(next_token(NULL, ','));
        pokemon_stats[i].base_stat = atoi(next_token(NULL, ','));
    }
    ...
    for (i = 1; i <= 9; i++) {
        fgets(line, 80, f);
        stats[i].id = atoi(next_token(line, ','));
